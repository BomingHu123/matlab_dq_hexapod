...
function vrep_hexapodscene_dqcontroller_pseudoinverse(simulation_parameters)
...
corin_controller = DQ_PseudoinverseController(corin);
...
corin_base = vi.get_object_pose('/hexapod');
...
%    pause(0.05);
    %% Compute control signal for the robot
    a = a+1;
    foothold1 = corin_hexapod.get_reference_feethold1_from_vrep;
    corin.set_reference_to_first_feethold_vrep(foothold1);
...
figure(4)
plot(T,corin_velocity_error_norm)
title('norm of corin velocity error')
xlabel('Time/s') 
ylabel('corin velocity error norm') 
