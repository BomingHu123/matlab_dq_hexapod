...
sampling_time = 0.5; % V-REP's sampling time is 50 ms.
...
for t=0:sampling_time:total_time
    ...
    last_corin_q = corin_q;
    a = a+1;
    % compute control signal
    foothold1 = corin_hexapod.get_reference_joint1_from_vrep;
    corin.set_reference_to_first_feethold_vrep(foothold1);
    [corin_u,norm_of_error] = corin_controller.compute_setpoint_control_signal(corin_q,Xd);
...
%     corin.set_base_frame(vi.get_object_pose('Hexapod'));

%     corin.update_base_frame(corin_q);
%     [x_current_abs_pose,base_velocity] = corin.get_estimated_base(corin_q,last_corin_q,t);
%     corin_q(9:26) = corin_q(9:26) + sampling_time*corin_u;
%     corin_q(1:8) = vec8(x_current_abs_pose);
%     corin.set_reference_to_first_feethold(corin_q);
